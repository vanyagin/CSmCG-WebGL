...
function main() {
  ...
  appShaderProgram = initShaderProgram(orangeVS, orangeFS);
  
  setupTextures();

  meshes.sphere = new OBJ.Mesh(sphere);
  for (let i=0; i<meshes.sphere.vertices.length; ++i) {
    meshes.sphere.vertices[i] *= 5;
  }
...
function initCommonUniforms(shaderProgram) {
  ...
  gl.uniform2fv(gl.getUniformLocation(shaderProgram,"uTextSize"),[300,300]);
...
function initObjBuffers(shaderProgram, meshObj) {
  ...
  gl.uniform3fv(gl.getUniformLocation(shaderProgram,"uColor"),[255/255, 125/255, 0/255]);
}
...
function setupTextures(){
  ...
  setTexture([image_list['orange.jpg']],
              [orangeTexture]);
}
...
function render(){
  ...
  gl.useProgram(appShaderProgram);
  initCommonUniforms(appShaderProgram);

  // Sphere
  initObjBuffers(appShaderProgram, meshes.sphere);
